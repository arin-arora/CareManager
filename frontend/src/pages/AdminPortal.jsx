...
export default function AdminPortal({ token }) {
  ...
  const loadLeaves = async (docId) => {
    try {
      // In prisma schema, doctorProfile includes leaves.
      // We can find the doctor in the local state or fetch details
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:5050'}/api/doctors/${docId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      const data = await response.json();
      // Wait, getDoctorDetails returns profile. Leaves can be fetched from admin list or separate route.
      // Let's check: /api/admin/doctors list already has doctor details.
      // We can also select from the doctors state array:
